...
async def reply_to_comment_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    comment_id = query.data.replace('reply_', '')
    ...
    keyboard = [[InlineKeyboardButton("❌ Cancel", callback_data="cancel_reply")]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.edit_message_text(
...
async def announcement_semester(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [[InlineKeyboardButton("❌ Cancel", callback_data="cancel_announcement")]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.edit_message_text(
...
async def handle_new_chat_members(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        ...
        if not message or not chat or chat.id != MAIN_CHANNEL_ID:
            return

        invite_link = getattr(message, 'invite_link', None)
        if not message.new_chat_members or not invite_link:
            return
